...
function ProductManagerLegacy({ categories }) {
  const emptyProduct = { categoria_id: "", codigo: "", nombre: "", precio: "", cantidad: "0", imagen_url: "", activo: true, afecto: false, tiene_caja: false, cantidad_caja: "", precio_caja: "", notificar_stock: false, stock_notificacion: "" };
  ...
  async function saveProduct(event) {
    ...
    if (form.notificar_stock) {
      const notifVal = Number(form.stock_notificacion);
      if (!form.stock_notificacion || isNaN(notifVal) || notifVal < 1) {
        setError("Para notificar stock, debes indicar un número de stock igual o mayor a 1.");
        return;
      }
    }
    setSaving(true);
    const payload = { ...form, nombre: form.nombre.trim().toUpperCase(), precio: Number(form.precio), cantidad: Number(form.cantidad), imagen_url: form.imagen_url || null, afecto: Boolean(form.afecto), tiene_caja: Boolean(form.tiene_caja), cantidad_caja: form.tiene_caja && form.cantidad_caja ? Number(form.cantidad_caja) : null, precio_caja: form.tiene_caja && form.precio_caja ? Number(form.precio_caja) : null, notificar_stock: Boolean(form.notificar_stock), stock_notificacion: form.notificar_stock && form.stock_notificacion ? Number(form.stock_notificacion) : null };
    try {
      if (editingProduct) {
...
function ProductManager({ categories }) {
  ...
  function openProduct(current = null) {
    setProduct(current);
    setForm(current ? { ...current, precio: String(current.precio), cantidad: String(current.cantidad), imagen_url: current.imagen_url ?? "", afecto: current.afecto ?? false, tiene_caja: current.tiene_caja ?? false, cantidad_caja: current.cantidad_caja != null ? String(current.cantidad_caja) : "", precio_caja: current.precio_caja != null ? String(current.precio_caja) : "", notificar_stock: current.notificar_stock ?? false, stock_notificacion: current.stock_notificacion != null ? String(current.stock_notificacion) : "" } : { ...blankProduct, categoria_id: categories.find((category) => category.activo)?.id ?? "" });
    setError("");
  }
...
function CustomerManager() {
  ...
  function openCustomer(customer = null) {
    ...
    setForm(customer ? { rut: customer.rut ?? "", nombre: customer.nombre ?? "", celular: customer.celular ?? "", correo: customer.correo ?? "", password: "", porcentaje: String(customer.porcentaje), dias_credito: String(customer.dias_credito ?? 0), activo: customer.activo } : blankCustomer);
    setError("");
    setCustomerFormOpen(true);
  }
  ...
  async function saveCustomer(event) {
    ...
    setSaving(true);
    const payload = { ...form, rut: form.rut.trim() || null, nombre: form.nombre.trim() || null, celular: form.celular.trim() || null, correo: form.correo.trim(), porcentaje: Number(form.porcentaje), dias_credito: Number(form.dias_credito || 0) };
...
function CreditManager() {
  ...
  const getDueDaysCount = (credit) => {
    if (!credit?.fecha_vencimiento) return 0;
    const due = new Date(credit.fecha_vencimiento);
    const now = new Date();
    // Comparación basada en la fecha a medianoche (hora local)
    const dueDate = new Date(due.getFullYear(), due.getMonth(), due.getDate());
    const todayDate = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    return Math.round((dueDate - todayDate) / 86_400_000);
  };

  const renderDueDaysBadge = (credit) => {
    if (paidFilter !== "pending") return "-";
    const days = getDueDaysCount(credit);
    let badgeClass = "badge-due badge-due-green";
    let label = `${days} d`;

    if (days >= 1) {
      badgeClass = "badge-due badge-due-green";
      label = days === 1 ? "1 día" : `${days} días`;
    } else if (days === 0) {
      badgeClass = "badge-due badge-due-yellow";
      label = "Hoy (0 d)";
    } else {
      badgeClass = "badge-due badge-due-red";
      label = `${days} d`;
    }
...
function AdminDashboard({ onLogout }) {
  ...
  const activeSection = isColaborador && !allowedColaboradorKeys.includes(section) ? "categories" : section;

  useEffect(() => {
    api.get("/admin/perfil")
      .then(({ data }) => {
        setAdminProfile(data);
        if (data?.rol?.nombre?.trim().toUpperCase() === "COLABORADOR") {
...
function Shop({ customer, onLogout, onProfileUpdated }) {
  ...
  useEffect(() => {
    ...
    [...history.querySelectorAll(".order-history-row")].forEach((row, index) => {
      ...
      date.textContent = order.created_at ? formatDateTime(order.created_at) : "-";
  ...
  useEffect(() => {
    ...
    root.render(<div className="modal-backdrop-custom"><section className="category-modal product-modal order-detail-modal" role="dialog" aria-modal="true"><header><div><p className="eyebrow">PEDIDO</p><h2>Detalle del pedido</h2></div><button className="icon-button" type="button" onClick={() => setSelectedOrder(null)} aria-label="Cerrar detalle"><X size={19} /></button></header><div className="modal-body-custom"><div className="order-detail-meta"><span>Pedido {selectedOrder.id.slice(0, 8).toUpperCase()}</span><span>{formatDateTime(selectedOrder.created_at, { dateStyle: "medium" })}</span></div><div className="order-detail-lines"><div><span>Producto</span><span>Cantidad</span><span>IVA</span><span>Precio</span><span>Subtotal</span></div>{(selectedOrder.detalles || []).map((line) => <div key={line.producto_id || line.id || Math.random()}><span>{line.nombre_producto}{line.tipo_empaque === "caja" ? <span className="badge bg-secondary ms-1" style={{ fontSize: "0.75rem" }}>Caja{line.cantidad_caja ? ` x${line.cantidad_caja}` : ""}</span> : null}</span><span>{line.cantidad} {line.tipo_empaque === "caja" ? (line.cantidad === 1 ? "cj." : "cjs.") : "un."}</span><span className={line.afecto ? "status-active" : "status-inactive"}>{line.afecto ? "Afecto" : "Exento"}</span><span>{money.format(line.precio_unitario)}</span><strong>{money.format(line.subtotal)}</strong></div>)}</div><div className="order-detail-total"><strong>Total</strong><strong>{money.format(selectedOrder.total)}</strong></div></div><footer><button className="btn btn-light" type="button" onClick={() => setSelectedOrder(null)}>Cerrar</button></footer></section></div>);
